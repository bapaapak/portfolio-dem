...
// Deploy commands
$commands = [
    "HOME=/tmp git config --global --add safe.directory {$basePath} 2>&1",
    "cd {$basePath} && HOME=/tmp git fetch origin 2>&1",
    "cd {$basePath} && HOME=/tmp git reset --hard origin/main 2>&1",
    "cd {$basePath} && HOME=/tmp git clean -fd --exclude=.env --exclude=storage 2>&1",
    ...
    "cd {$basePath} && rm -f bootstrap/cache/*.php 2>&1 || true",
    "cd {$basePath} && chmod -R 777 bootstrap/cache 2>&1 || true",
