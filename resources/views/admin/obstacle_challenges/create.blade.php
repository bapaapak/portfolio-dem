...
@section('title', 'Tambah Obstacle/Challenge')

@section('content')
<div class="mb-6 flex justify-between items-center">
    <div>
        <h1 class="text-2xl font-bold text-gray-800">Tambah Obstacle/Challenge</h1>
        <p class="text-gray-600 mt-1">Add a new obstacle or challenge to your portfolio.</p>
    </div>
...
<div class="bg-white rounded-xl shadow-sm border border-gray-100 overflow-hidden">
    <div class="p-6">
        <form action="{{ route('admin.obstacle-challenges.store') }}" method="POST">
            @csrf
            ...
            <div class="grid grid-cols-1 md:grid-cols-2 gap-6 mb-6">
                <div>
                    ...
                    <select name="type" id="type" class="w-full rounded-lg border-gray-300 bg-white text-gray-900 focus:border-indigo-500 focus:ring focus:ring-indigo-200 focus:ring-opacity-50 transition-shadow @error('type') border-red-500 @enderror" required>
                        <option value="obstacle" {{ old('type') == 'obstacle' ? 'selected' : '' }}>Obstacle</option>
                ...
                <div>
                    ...
                    <input type="number" name="order" id="order" class="w-full rounded-lg border-gray-300 bg-white text-gray-900 focus:border-indigo-500 focus:ring focus:ring-indigo-200 focus:ring-opacity-50 transition-shadow @error('order') border-red-500 @enderror" value="{{ old('order', 0) }}" min="0">
                    @error('order')
                    <p class="mt-1 text-sm text-red-600">{{ $message }}</p>
                    @enderror
                </div>
            ...
            <div class="mb-8">
                <div class="flex items-center">
                    <input type="checkbox" name="is_active" id="is_active" class="rounded border-gray-300 text-indigo-600 shadow-sm focus:border-indigo-300 focus:ring focus:ring-indigo-200 focus:ring-opacity-50 w-4 h-4" value="1" {{ old('is_active', true) ? 'checked' : '' }}>
