    ...
    <div class="p-6">
        <form action="{{ route('admin.obstacle-challenges.update', $obstacleChallenge) }}" method="POST">
            ...
            <div class="mb-6">
                ...
                <input type="text" name="title_en" id="title_en" class="w-full rounded-lg border-gray-300 bg-white text-gray-900 focus:border-indigo-500 focus:ring focus:ring-indigo-200 focus:ring-opacity-50 transition-shadow @error('title_en') border-red-500 @enderror" value="{{ old('title_en', $obstacleChallenge->title_en) }}" placeholder="English title (optional)">
                @error('title_en')
                <p class="mt-1 text-sm text-red-600">{{ $message }}</p>
                @enderror
            </div>
            ...
            <div class="mb-6">
                ...
                <div id="items-en-container" class="space-y-3 mb-3">
                    @if($obstacleChallenge->items_en && count($obstacleChallenge->items_en) > 0)
                        @foreach($obstacleChallenge->items_en as $item)
                        <div class="flex items-center gap-2 group">
                            <input type="text" name="items_en[]" class="flex-1 rounded-lg border-gray-300 bg-white text-gray-900 focus:border-indigo-500 focus:ring focus:ring-indigo-200 focus:ring-opacity-50 transition-shadow" value="{{ $item }}" placeholder="Item detail (English)...">
                            <button type="button" class="text-red-500 hover:text-red-700 p-2 opacity-50 hover:opacity-100 transition-opacity" onclick="this.closest('.flex').remove()">
                                <i class="fas fa-times"></i>
                            </button>
                        </div>
