    ...
    <form action="{{ route('admin.experiences.store') }}" method="POST" enctype="multipart/form-data">
            ...
            <div class="form-group">
                ...
                <div style="display: flex; gap: 15px; margin-top: 5px;">
                    <label style="cursor: pointer;">
                        <input type="radio" name="date_format" value="F Y" checked> Month & Year (e.g. Jan 2024)
                    </label>
                    <label style="cursor: pointer;">
                        <input type="radio" name="date_format" value="Y"> Year Only (e.g. 2024)
                    </label>
                </div>
            </div>
        </div>
        ...
        <div class="form-row">
            <div class="form-group">
                ...
                <input type="text" id="start_date" name="start_date" class="form-control datepicker" value="{{ old('start_date') }}" required placeholder="YYYY-MM-DD">
            </div>
            <div class="form-group">
                <label for="end_date">End Date</label>
                <input type="text" id="end_date" name="end_date" class="form-control datepicker" value="{{ old('end_date') }}" placeholder="Leave blank if current job">
            </div>
        </div>
        ...
        <div class="form-group">
            ...
            <label class="form-check" style="margin-left: 15px;">
                <input type="checkbox" name="show_description" value="1" {{ old('show_description', true) ? 'checked' : '' }}>
                <span>Show Description</span>
            </label>
            <label class="form-check" style="margin-left: 15px;">
                <input type="checkbox" name="show_tags" value="1" {{ old('show_tags', true) ? 'checked' : '' }}>
        ...
        <div class="form-group">
            ...
            <input type="text" id="technologies" name="technologies" class="form-control" value="{{ old('technologies') }}" placeholder="React, TypeScript, Laravel">
        ...
        <div class="form-group" style="margin-bottom: 20px;">
            ...
            <div style="position: relative;">
                <textarea name="description" id="description" rows="5" class="form-control" style="width: 100%; padding: 10px; border-radius: 6px; border: 1px solid var(--border-color); background: var(--bg-input); color: var(--text-primary);">{{ old('description') }}</textarea>
        ...
        <div class="form-group" style="margin-bottom: 20px; padding: 15px; background: rgba(0,0,0,0.02); border-radius: 8px; border: 1px dashed #cbd5e1;">
            ...
            <div class="form-row">
                <div class="form-group">
                    ...
                    <input type="text" id="title_en" name="title_en" class="form-control" value="{{ old('title_en') }}" placeholder="e.g. Senior Frontend Engineer">
                </div>
                <div class="form-group">
                    <label for="company_en">Company (EN)</label>
                    <input type="text" id="company_en" name="company_en" class="form-control" value="{{ old('company_en') }}" placeholder="e.g. TechStart Inc">
                </div>
            </div>
             ...
             <div class="form-row">
                <div class="form-group">
                    ...
                    <input type="text" id="location_en" name="location_en" class="form-control" value="{{ old('location_en') }}" placeholder="e.g. Jakarta, Indonesia">
                </div>
            </div>

            <div class="form-group">
                <label for="technologies_en">Technologies (Badges - EN)</label>
                <input type="text" id="technologies_en" name="technologies_en" class="form-control" value="{{ old('technologies_en') }}" placeholder="React, TypeScript, Laravel">
                <small class="form-text" style="color: var(--text-secondary);">English translation for badges</small>
            </div>

            <div class="form-group">
                <label for="description_en">Description (EN)</label>
                <textarea name="description_en" id="description_en" rows="5" class="form-control" style="width: 100%; padding: 10px; border-radius: 6px; border: 1px solid var(--border-color); background: var(--bg-input); color: var(--text-primary);">{{ old('description_en') }}</textarea>
        ...
        <script>
            ...
            document.getElementById('generate-desc-btn').addEventListener('click', function() {
                ...
                .then(data => {
                    if (data.description) {
                        // Append or replace? Let's append if not empty, or replace if empty.
                        // Actually, user usually wants it filled. Let's replace but confirm if not empty?
                        // For simplicity, just set value, or append if user wants multiple?
