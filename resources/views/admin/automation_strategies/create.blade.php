...
@section('title', 'Tambah Strategi')

@section('content')
<div class="content-header">
    <div class="header-left">
        <h1>Tambah Strategi</h1>
        <p class="subtitle">Tambahkan strategi otomasi & digitalisasi baru</p>
    </div>
...
<div class="form-card">
    <form action="{{ route('admin.automation-strategies.store') }}" method="POST">
        @csrf
        ...
        <div class="form-section">
            ...
            <div class="form-row">
                <div class="form-group">
                    ...
                    <select id="term_type" name="term_type" class="form-control" required>
                        <option value="short" {{ old('term_type') == 'short' ? 'selected' : '' }}>Short Term Strategy</option>
                        <option value="middle" {{ old('term_type') == 'middle' ? 'selected' : '' }}>Middle Term Strategy</option>
                        <option value="long" {{ old('term_type') == 'long' ? 'selected' : '' }}>Long Term Strategy</option>
                    </select>
                    @error('term_type')
                ...
                <div class="form-group">
                    ...
                    <select id="category" name="category" class="form-control" required>
                        ...
                        <option value="digitalization" {{ old('category') == 'digitalization' ? 'selected' : '' }}>Digitalization & Automation</option>
                    </select>
                    @error('category')
                    <div class="text-sm text-red-500 mt-1">{{ $message }}</div>
                    @enderror
            ...
            <div class="form-group">
                ...
                <input type="text" id="title" name="title" class="form-control" value="{{ old('title') }}" placeholder="Contoh: Develop Plant 1 for AEP, PES & AMRS (opsional)">
                @error('title')
                <div class="text-sm text-red-500 mt-1">{{ $message }}</div>
                @enderror
            </div>
