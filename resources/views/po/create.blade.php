...
@section('content')
    <div class="mb-4">
        <h3 class="fw-bold text-dark mb-1">Create Purchase Order</h3>
        <p class="text-muted mb-0">Form to generate a new PO, optionally from an approved PR.</p>
    </div>
                    ...
                    <div class="col-md-4">
                        ...
                        <select name="vendor_id" class="form-select" required>
                            ...
                            @foreach($vendors as $vendor)
                                <option value="{{ $vendor->id }}">{{ $vendor->vendor_code }} - {{ $vendor->vendor_name }}
                                </option>
                            @endforeach
                        </select>
                    </div>
                ...
                <div class="mb-4">
                    ...
                    <div class="table-responsive">
                        <table class="table table-bordered table-sm align-middle mb-0" id="itemsTable">
                            ...
                            <tbody id="itemsBody">
                                @if(!empty($prItems) && count($prItems) > 0)
                                        ...
                                        <tr class="item-row">
                                            ...
                                            <td>
                                                <input type="number" step="0.01"
                                                    ...
                                                    name="items[{{ $idx }}][qty]" value="{{ $item->qty_req }}"
                                            ...
                                            <td>
                                                <input type="number" step="0.01"
                                                    ...
                                                    name="items[{{ $idx }}][unit_price]" value="{{ $item->estimated_price }}"
                                                    oninput="calcTotals()" required>
                                            </td>
                                            <td>
                                                <input type="number" step="0.1"
                                                    class="form-control form-control-sm item-tax text-end"
                                                    name="items[{{ $idx }}][tax_percent]" value="0" oninput="calcTotals()">
                                            </td>
                                            <td>
                                                <input type="text"
                                            ...
                                            <td>
                                                <input type="text" class="form-control form-control-sm"
                                                    name="items[{{ $idx }}][notes]" value="PR: {{ $prNumber }}" readonly>
                                            </td>
                                ...
                                @else
                                    ...
                                    <tr class="item-row">
                                        <td><input type="text" class="form-control form-control-sm"
                                                name="items[0][item_description]" required></td>
                                        <td><input type="number" step="0.01"
                                                class="form-control form-control-sm item-qty text-end" name="items[0][qty]"
                                                value="1" oninput="calcTotals()" required></td>
                                        <td><input type="text" class="form-control form-control-sm" name="items[0][uom]"></td>
                                        <td><input type="number" step="0.01"
                                                class="form-control form-control-sm item-price text-end"
                                                name="items[0][unit_price]" value="0" oninput="calcTotals()" required></td>
                                        <td><input type="number" step="0.1"
                                                class="form-control form-control-sm item-tax text-end"
                                                name="items[0][tax_percent]" value="0" oninput="calcTotals()"></td>
                                        <td><input type="text"
                                                class="form-control form-control-sm item-total bg-light text-end fw-bold"
                                                readonly value="0"></td>
                                        <td><input type="text" class="form-control form-control-sm" name="items[0][notes]"></td>
                                        <td class="text-center"><button type="button"
                                                class="btn btn-sm btn-link text-danger p-0" onclick="removeRow(this)"><i
                ...
                <div class="row">
                    <div class="col-md-6 mb-3">
                        ...
                        <input type="text" name="delivery_terms" class="form-control"
                            placeholder="e.g. FOB Destination, Ex-Works">
                    </div>
                    <div class="col-md-6 mb-3">
                        <label class="form-label">Plant/Delivery Address Info</label>
                        <input type="text" name="plant" class="form-control" placeholder="Plant location or detail">
                    </div>
                    <div class="col-12 mb-3">
                        <label class="form-label">Additional Notes</label>
                        <textarea name="notes" class="form-control" rows="3"
                            placeholder="Additional conditions, warranties, etc."></textarea>
                    </div>
                </div>

                <div class="d-flex justify-content-end gap-2 mt-4 pt-3 border-top">
                    <a href="{{ route('po.index') }}" class="btn btn-light px-4 border">Cancel</a>
                    <button type="submit" class="btn btn-primary px-5 fw-bold shadow-sm">Save as Draft PO</button>
                </div>
            </form>
        </div>
    </div>
...
@push('scripts')
    <script>
        let rowIndex = {{ !empty($prItems) ? count($prItems) : 1 }};
